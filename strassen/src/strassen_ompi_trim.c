...
/* (l46) typedef double REAL; */

/* (l47) typedef unsigned long PTR; */


void init_matrix(int n, double (* A), int an);
...
void FastNaiveMatrixMultiply(double (* C), double (* A), double (* B), unsigned MatrixSize, unsigned RowWidthC, unsigned RowWidthA, unsigned RowWidthB);
void FastAdditiveNaiveMatrixMultiply(double (* C), double (* A), double (* B), unsigned MatrixSize, unsigned RowWidthC, unsigned RowWidthA, unsigned RowWidthB);
void MultiplyByDivideAndConquer(double (* C), double (* A), double (* B), unsigned MatrixSize, unsigned RowWidthC, unsigned RowWidthA, unsigned RowWidthB, int AdditiveMode);
void OptimizedStrassenMultiply_par(double (* C), double (* A), double (* B), unsigned MatrixSize, unsigned RowWidthC, unsigned RowWidthA, unsigned RowWidthB, int Depth);
void OptimizedStrassenMultiply_seq(double (* C), double (* A), double (* B), unsigned MatrixSize, unsigned RowWidthC, unsigned RowWidthA, unsigned RowWidthB, int Depth);
...
void FastNaiveMatrixMultiply(double (* C), double (* A), double (* B), unsigned MatrixSize, unsigned RowWidthC, unsigned RowWidthA, unsigned RowWidthB)
{
  unsigned long RowWidthBInBytes = RowWidthB << 3;
  unsigned long RowWidthAInBytes = RowWidthA << 3;
  unsigned long MatrixWidthInBytes = MatrixSize << 3;
  unsigned long RowIncrementC = (RowWidthC - MatrixSize) << 3;
...
{
  unsigned QuadrantSize = MatrixSize >> 1;
  unsigned QuadrantSizeInBytes = sizeof(double ) * QuadrantSize * QuadrantSize + 32;
  unsigned Column, Row;
  double (* A12);
  double (* B12);
  double (* C12);
  ...
  unsigned long TempMatrixOffset = 0;
  unsigned long MatrixOffsetA = 0;
  unsigned long MatrixOffsetB = 0;
  char * Heap;
  void * StartHeap;
  unsigned long RowIncrementA = (RowWidthA - QuadrantSize) << 3;
  unsigned long RowIncrementB = (RowWidthB - QuadrantSize) << 3;
  unsigned long RowIncrementC = (RowWidthC - QuadrantSize) << 3;
  ...
  if (((unsigned long ) Heap) & 31)
    Heap = (char *) (((unsigned long ) Heap) + 32 - (((unsigned long ) Heap) & 31));
  S1 = (double (*)) Heap;
  ...
  for (Row = 0; Row < QuadrantSize; Row++)
    {
      for (Column = 0; Column < QuadrantSize; Column += 4)
        {
          double LocalM5_0 = *(M5);
          double LocalM5_1 = *(M5 + 1);
          double LocalM5_2 = *(M5 + 2);
          double LocalM5_3 = *(M5 + 3);
          double LocalM2_0 = *(M2);
          double LocalM2_1 = *(M2 + 1);
          double LocalM2_2 = *(M2 + 2);
          double LocalM2_3 = *(M2 + 3);
          double T1_0 = *(T1sMULT) + LocalM2_0;
          double T1_1 = *(T1sMULT + 1) + LocalM2_1;
          double T1_2 = *(T1sMULT + 2) + LocalM2_2;
          double T1_3 = *(T1sMULT + 3) + LocalM2_3;
          double T2_0 = *(C22) + T1_0;
          double T2_1 = *(C22 + 1) + T1_1;
          double T2_2 = *(C22 + 2) + T1_2;
          double T2_3 = *(C22 + 3) + T1_3;
      ...
      C = (double (*)) (((unsigned long ) C) + RowIncrementC);
      C12 = (double (*)) (((unsigned long ) C12) + RowIncrementC);
      C21 = (double (*)) (((unsigned long ) C21) + RowIncrementC);
      C22 = (double (*)) (((unsigned long ) C22) + RowIncrementC);
    }
  free(StartHeap);
}
...
/* Outlined code for (l1154) #pragma omp task untied */
...
static void * _taskFunc6_(void * __arg)
{
  ...
  /* byvalue variables */
  double (* C21) = _tenv->C21;
  double (* A22) = _tenv->A22;
  double (* S8) = _tenv->S8;
  unsigned QuadrantSize = _tenv->QuadrantSize;
  unsigned RowWidthC = _tenv->RowWidthC;
  unsigned RowWidthA = _tenv->RowWidthA;
  int Depth = _tenv->Depth;

  /* byresult variables */
  /* (l1154) #pragma omp task untied -- body moved below */
...
/* Outlined code for (l1150) #pragma omp task untied */
...
static void * _taskFunc5_(void * __arg)
{
  ...
  /* byvalue variables */
  double (* C12) = _tenv->C12;
  double (* S4) = _tenv->S4;
  double (* B22) = _tenv->B22;
  unsigned QuadrantSize = _tenv->QuadrantSize;
  unsigned RowWidthC = _tenv->RowWidthC;
  unsigned RowWidthB = _tenv->RowWidthB;
  int Depth = _tenv->Depth;

  /* byresult variables */
  /* (l1150) #pragma omp task untied -- body moved below */
...
/* Outlined code for (l1146) #pragma omp task untied */
...
static void * _taskFunc4_(void * __arg)
{
  ...
  /* byvalue variables */
  double (* C) = _tenv->C;
  double (* A12) = _tenv->A12;
  double (* B21) = _tenv->B21;
  unsigned QuadrantSize = _tenv->QuadrantSize;
  unsigned RowWidthC = _tenv->RowWidthC;
  unsigned RowWidthA = _tenv->RowWidthA;
  unsigned RowWidthB = _tenv->RowWidthB;
  ...
  /* byresult variables */
  /* (l1146) #pragma omp task untied -- body moved below */
...
/* Outlined code for (l1142) #pragma omp task untied */
...
static void * _taskFunc3_(void * __arg)
{
  ...
  /* byresult variables */
  /* (l1142) #pragma omp task untied -- body moved below */
...
/* Outlined code for (l1138) #pragma omp task untied */
...
static void * _taskFunc2_(void * __arg)
{
  ...
  /* byresult variables */
  /* (l1138) #pragma omp task untied -- body moved below */
...
/* Outlined code for (l1134) #pragma omp task untied */
...
static void * _taskFunc1_(void * __arg)
{
  ...
  /* byresult variables */
  /* (l1134) #pragma omp task untied -- body moved below */
...
/* Outlined code for (l1130) #pragma omp task untied */
...
static void * _taskFunc0_(void * __arg)
{
  ...
  /* byresult variables */
  /* (l1130) #pragma omp task untied -- body moved below */
...
/* Outlined code for (l1270) #pragma omp parallel  */
...
static void * _thrFunc0_(void * __arg)
{
  ...
  /* byresult variables */
  /* (l1270) #pragma omp parallel  -- body moved below */

  {
    /* (l1271) #pragma omp single  */

    if (ort_mysingle(1))
      _taskFunc7_((void *)0);
    ort_leaving_single();
  }
  CANCEL_parallel_1270 :
    ort_taskwait(2);
  return ((void *) 0);
}

/* Outlined code for (l1272) #pragma omp task untied */
...
static void * _taskFunc7_(void * __arg)
{
  ...
  /* byresult variables */
  /* (l1272) #pragma omp task untied -- body moved below */
