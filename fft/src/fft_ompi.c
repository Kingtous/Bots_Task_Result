...
void fft_twiddle_gen_1(struct _noname19_ (* in), struct _noname19_ (* out), struct _noname19_ (* W), int r, int m, int nW, int nWdnti, int nWdntm)
{
  int j, k;
  struct _noname19_ (* jp);
      ...
      if (omp_in_final() || ort_task_throttling())
        {
          ...
          {
            # 207 "fft.c"
            fft_twiddle_gen_1(in + i, out + i, W, r, m, nW, nWdn * i, nWdn * m);
            CANCEL_task_205 :
              ;
          }
          ort_task_immediate_end(_itn);
