...
void initialize()
{
  ...
  nodes = (struct node (*)) malloc(bots_arg_size * sizeof(struct node ));
  ...
  if (visited[i] == 0)
    {
      ...
      for (j = 0; j < nodes[i].n; j++)
        {
          ...
          if (omp_in_final() || ort_task_throttling())
            ...
            /* (l106) #pragma omp task untied firstprivate(i, cc) */
            {
              struct __taskenv__ {
                  int cc;
                  int n;
      ...
      /* (l109) #pragma omp taskwait  */
      # 109 "concom.c"
      ort_taskwait(0);
    }
}

/* Outlined code for (l106) #pragma omp task untied firstprivate(i, cc) */
...
static void * _taskFunc0_(void * __arg)
{
  ...
  /* byresult variables */
  /* (l106) #pragma omp task untied firstprivate(i, cc) -- body moved below */
  # 106 "concom.c"
  # 106 "concom.c"
  {
    CC_par(n, cc);
...
/* Outlined code for (l146) #pragma omp parallel  */
...
static void * _thrFunc0_(void * __arg)
{
  ...
  /* byresult variables */
  /* (l146) #pragma omp parallel  -- body moved below */
  # 146 "concom.c"
  {
    /* (l147) #pragma omp single  */
    # 147 "concom.c"
    if (ort_mysingle(1))
      if (omp_in_final() || ort_task_throttling())
        {
                ...
                if (visited[(*i)] == 0)
                  {
                    if (omp_in_final() || ort_task_throttling())
                      ...
                      /* (l153) #pragma omp task untied firstprivate(i, cc) */
                      {
                        struct __taskenv__ {
                            int i;
                            int * cc;
...
/* Outlined code for (l148) #pragma omp task untied */
...
static void * _taskFunc2_(void * __arg)
{
  ...
  /* byresult variables */
  /* (l148) #pragma omp task untied -- body moved below */
  # 148 "concom.c"
  {
    # 158 "concom.c"
    for ((*i) = 0; (*i) < bots_arg_size; (*i)++)
...
/* Outlined code for (l153) #pragma omp task untied firstprivate(i, cc) */
...
static void * _taskFunc1_(void * __arg)
{
  ...
  /* byresult variables */
  /* (l153) #pragma omp task untied firstprivate(i, cc) -- body moved below */
  # 153 "concom.c"
  # 153 "concom.c"
  {
    CC_par(i, *cc);
